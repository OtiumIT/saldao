...
import * as movimentacoesRepoSupabase from './movimentacoes.repository.supabase.js';
...
export const movimentacoesRoutes = new Hono<Ctx>()
  .get('/', async (c) => {
    ...
    try {
      const list = useSupabaseDataAPI(c.env)
        ? await movimentacoesRepoSupabase.list(c.env, {
  ...
  .post('/', async (c) => {
    ...
    try {
      const created = useSupabaseDataAPI(c.env)
        ? await movimentacoesRepoSupabase.create(c.env, parsed.data)
        : await movimentacoesRepo.create(parsed.data);
  ...
  .post('/ajuste', async (c) => {
    ...
    try {
      const created = useSupabaseDataAPI(c.env)
        ? await movimentacoesRepoSupabase.ajuste(
            c.env,
        ...
        : await movimentacoesRepo.ajuste(
            parsed.data.produto_id,
            parsed.data.quantidade,
            parsed.data.observacao,
            parsed.data.cor_id ?? undefined
  ...
  .post('/conferencia', async (c) => {
    ...
    try {
      const result = useSupabaseDataAPI(c.env)
        ? await movimentacoesRepoSupabase.conferenciaLote(c.env, parsed.data.itens)
        : await movimentacoesRepo.conferenciaLote(parsed.data.itens);
