...
export const producaoRoutes = new Hono<Ctx>()
  .get('/bom/:produtoFabricadoId', async (c) => {
    ...
    try {
      const list = await producaoService.listBomByFabricado(id);
      return c.json(list);
    } catch (e) {
  ...
  .post('/bom', async (c) => {
    ...
    try {
      const created = await producaoService.saveBomItem(parsed.data);
  ...
  .delete('/bom/:produtoFabricadoId/:produtoInsumoId', async (c) => {
    ...
    try {
      const ok = await producaoService.removeBomItem(fabricadoId, insumoId);
  ...
  .get('/quantidade-possivel/:produtoFabricadoId', async (c) => {
    ...
    const id = c.req.param('produtoFabricadoId');
    try {
      const result = await producaoService.quantidadePossivel(id);
  ...
  .get('/ordens', async (c) => {
    ...
    try {
      const list = await producaoService.listOrdens({ status, data_inicio, data_fim });
      return c.json(list);
    } catch (e) {
  ...
  .get('/ordens/:id/itens', async (c) => {
    ...
    try {
      const itens = await producaoService.listOrdensItens(id);
      return c.json(itens);
    } catch (e) {
  ...
  .post('/ordens', async (c) => {
    ...
    if (!parsed.success) return c.json({ error: parsed.error.flatten().fieldErrors }, 400);
    try {
      const created = await producaoService.createOrdem(parsed.data);
      return c.json(created, 201);
    } catch (e) {
  ...
  .get('/conferir-estoque-por-cor', async (c) => {
    ...
    if (!parsed.success) return c.json({ error: parsed.error.flatten().fieldErrors }, 400);
    try {
      const result = await producaoService.conferirEstoquePorCor(parsed.data);
      return c.json(result);
    } catch (e) {
  ...
  .post('/ordens/:id/executar', async (c) => {
    ...
    const id = c.req.param('id');
    try {
      const result = await producaoService.executarOrdem(id);
